...
let commit ?(difficulty = 7) ?(force = false) commit_file open_file =
  ...
  if (not force) && Sys.file_exists commit_file then cant_overwrite commit_file
    ...
    let c, o = Nocoiner.commit ~difficulty secret in
...
let break commit_file =
  let c = read_from commit_file in
  let secret = Nocoiner.break c in
  print_endline secret ; flush stdout
